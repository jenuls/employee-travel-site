...
<%
	...
	while(it.hasNext() )
	{
		...
		relation = strUtil.getPara(hm.get("relatives"));	
	}
%>
...
<body>
<form name="myForm" action="relativesDataAction.jsp" method="post">
	<div id="page" align="center">
		<div id="content" style="width:800px">
			...
			<div id="submenu">
				<div align="right" class="smallgraytext" style="padding:9px;"><a href="personalData.jsp">個人資料</a> | <a href="relativesDataList.jsp">親屬資料</a>
        	</div>
			</div>
...
<%
	...
	{
%> 
			 	<tr><td>目前無親屬資料，請新增，謝謝</td></tr>
<%
	 }else{                  
%>	  
               <tr>
                    <td class='FormTDH' style='width:20%'>關係</td><td class='FormTDH' style='width:30%'><input id='relation' name='relation' type='radio' <%if(relation.equals("直系") ) out.print("checked"); %> value='直系'>直系&nbsp;&nbsp;<input id='relation02' name='relation' type='radio' value='親友' <%if(relation.equals("親友") ) out.print("checked"); %>>親友</td>
                    <td class='FormTDH' style='width:20%'>身份證字號</td><td class='FormTDH' style='width:30%'><input id='id' name='id' type='text' style='width:90%' value='<%=idNumber%>' readonly onClick="alert('該欄位不允許修改，若要修改此欄位，請直接刪除此資料，重新新增');"></td>
                </tr>
                <tr>
...
%>
        ...
        <div style="font-size:65%">
          <input type="button" onclick="document.forms[0].submit();" value="確定"/>&nbsp;&nbsp;&nbsp;<input type="button" onclick="window.location.href='relativesDataList.jsp'" value="取消"/>
          <br /><br />
        </div>	
        <div style="border:1px solid #333333;width:100%"></div>
			</div>
...
function submitFn(){
			if( $(":radio[name='relation']:checked").size() == 0   )
			{
				alert("請填寫關係，謝謝。");
				return false;
			}
