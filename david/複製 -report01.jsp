...
<%
	...
	for(int i=0;i<al01.size();i++){ 
		...
		if(al06.size()!=0){
			...
			al07 = connObj.select("SELECT idNumber,birthday FROM relativesdata WHERE employeeNumber ='" + arr01[2] + "' and name='" + arr01[3] + "'");
...
try {
  ...
  OutputStream os = response.getOutputStream();
	...
	writableSheet.addCell(label);
	label = new Label(4, 0, "身分證字號",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(5, 0, "年齡",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(6, 0, "生日",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(7, 0, "房間",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(8, 0, "餐桌",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(9, 0, "車輛",writableCellFormat);
	writableSheet.addCell(label);
	label = new Label(10, 0, "費用",writableCellFormat);
	writableSheet.addCell(label);
	...
	/*
		...
		*/
		...
		writableSheet.addCell(label);
		label = new Label(4, i+1, idArr[i],writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(5, i+1, ageArr[i],writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(6, i+1, birthdayArr[i],writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(7, i+1, roomNoArr[i] + "【" + roomDetailNoArr[i] + "】",writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(8, i+1, deskNoArr[i] + "【" + deskDetailNoArr[i] + "】",writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(9, i+1, vehicleNoArr[i] + "【" + vehicleDetailNoArr[i] + "】",writableCellFormat);
		writableSheet.addCell(label);
		label = new Label(10, i+1, String.valueOf(Integer.parseInt(moneyArr[i]) + Integer.parseInt(travelMoneyArr[i])),writableCellFormat);
		writableSheet.addCell(label);
	}
